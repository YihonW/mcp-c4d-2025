...
本记录保留早期安装、源码修复和失败验证的历史。最新进展：`4a62967` 已于 2026-09-06 通过 foundation 和限定范围的 Redshift 严格真实测试，详见[后续自主维护记录](./2026-09-06-autonomous-validation.md)。这不代表所有工具或所有 Redshift 选项已经验证。
...
本记录中的等待安装事项已由后续自主维护轮接续处理；18:55 的两套严格测试均通过，测试工程和标记均已清理，用户可以正常使用 C4D。后续扩展仍应先只读检查，再在暂停场景操作的窗口使用单个临时工程验证，不运行全部历史 E2E。
